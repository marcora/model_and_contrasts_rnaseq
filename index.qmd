...
```{r}
(design = model.matrix(expression ~ 1 + group, data))
```

```{r}
fit = lm(expression ~ 1 + group, data)
tidy(fit)
```
...
```{r}
(design = model.matrix(expression ~ 0 + group, data))
```

```{r}
fit = lm(expression ~ 0 + group, data)
tidy(fit)
```
...
contrasts = makeContrasts(groupKO - groupWT, levels=colnames(design))
...
contrasts = makeContrasts(treatmentA - treatmentCTRL, levels=colnames(design))
...
contrasts = makeContrasts(treatmentB - treatmentA, levels=colnames(design))
...
cells = treat2
levels(cells) = c("B", "T")
treatment = paste(tissue, cells, sep=":")
treatment = as_factor(treatment)
data = tibble(mouse, expression, tissue, cells, treatment)
data
```
...
```{r}
avg_predictions(fit, variables = c("treatment"), df = insight::get_df(fit))
```
